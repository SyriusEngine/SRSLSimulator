...
namespace SrslAPI{
    ...
    void ColorAttachment::clear(float r, float g, float b, float a) {
        for (uint32_t i = 0; i < m_Data.size(); i += m_ChannelCount){
            m_Data[i] = static_cast<uint8_t>(r * 255);
            m_Data[i + 1] = static_cast<uint8_t>(g * 255);
            m_Data[i + 2] = static_cast<uint8_t>(b * 255);
            m_Data[i + 3] = static_cast<uint8_t>(a * 255);
        }
